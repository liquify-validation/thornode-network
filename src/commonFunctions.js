...
export const getData = async () => {
  ...
  const churnsInYear =
    365 /
    ((globalData.secondsPerBlock * globalData.churnInterval) / 60 / 60 / 24);
  ...
  val.data.map((item) => {
    item.age =
      ((globalData.maxHeight - item.status_since) * blockTime) / 60 / 60 / 24;
    item.score = (globalData.blocksSinceLastChurn / item.slash_points).toFixed(
      1
    );
    ...
    item.obchains =
      item.observe_chains === null
        ? 0
        : item.observe_chains.reduce(
            (obj, item) => Object.assign(obj, { [item.chain]: item.height }),
            {}
          );
    item.apy =
      (
  ...
  const maxBaseHeight = reduceDown(data2, "BASE");
  const maxTronHeight = reduceDown(data2, "TRON");
...
export const API_Call = async (myInit) => {
  ...
  try {
    ...
    if (!process.env.NODE_ENV || process.env.NODE_ENV === "development") {
      console.log(
        "DEV ONLY: Returned data from API call: ",
        myInit.body.operation,
        val
      );
    }
    ...
    if (val.errorMessage) {
      console.log(
        "is an unhandled error - " + myInit.body.operation + ":",
        val
      );
      return false;
    }
    ...
    if (val.success === false) {
      console.log(
        "is an error from success - " + myInit.body.operation + ":",
        val
      );
      return false;
    }
